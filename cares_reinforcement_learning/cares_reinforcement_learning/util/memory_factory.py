    ...
    def create_memory(self, memory_type, buffer_size, **kwargs):
        ...
        if memory_type == "PER":
            if not 'observation_size' in kwargs:
                raise AttributeError('observation_size not passed into kwargs. PER memory buffer is dependent on this.')
        ...
        if memory_type == "EpisodicBuffer":
            
            return MemoryBuffer(max_capacity=buffer_size), PrioritizedReplayBuffer(kwargs['observation_size'], kwargs['action_num'], max_size=buffer_size), MemoryBuffer(max_capacity=buffer_size)
        
        raise ValueError(f"Unkown memory type: {memory_type}")
    
    
    def create_episode_memories(self, memory_type, buffer_size, **kwargs):
        if memory_type == "MemoryBuffer":
            return MemoryBuffer(max_capacity=buffer_size)
        if memory_type == "PER":
            if not 'observation_size' in kwargs:
